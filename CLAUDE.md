...
- **Retire, don't delete.** Any artifact that has accumulated incoming references (personas, stories DS-NNN, screens P-/OV-/DE-, business rules BR-NN, principles GP-NNN, patterns PA-NNN, canvas briefs, interaction specs, components, wiki pages) is retired by adding `status: retired` to its frontmatter OR a `[retired]` marker on its heading OR the word `retired` on its registry row — never by deleting the file or reusing its ID. `sync-traceability.js` and `sync-wiki.js` honor the markers; retired artifacts are excluded from orphan checks but remain cite-able. See "Retirement status convention" in `design/process/README.md`.
