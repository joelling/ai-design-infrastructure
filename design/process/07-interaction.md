...
- `design/06_INFORMATION_ARCHITECTURE/notifications.md` — canonical NOTIF-NNN catalog; this mode references notifications by id, never by inline copy
...
**Notifications by reference, not by inline copy.** When an error or behavior triggers a notification (toast, email, push, SMS, system message), reference the notification by its `[NOTIF-NNN]` tag from `design/06_INFORMATION_ARCHITECTURE/notifications.md`. Never carry the message text inline in `error-strategy.md` or `behavioral-spec.md` — the notification catalog is the canonical owner. If a needed notification doesn't exist yet, add it to `notifications.md` first, then reference the new id here.
...
This mode does not write to the BRD directly. AC enrichment flows through `story-map.md`:

1. **Acceptance criteria** — for each story with behavioral specs, append new bullet points to the AC list in `design/05_STORIES/story-map.md`. Each bullet captures one state or behavior requirement, with the source tag inline at the end:
   - `User receives confirmation [NOTIF-012] after successful submission  [STATE]`
   - `System prevents submission when required fields are incomplete  [BEHAVIOR]`
   - Notification references use `[NOTIF-NNN]` tags pointing at `06_INFORMATION_ARCHITECTURE/notifications.md`
2. **Notification Mapping sheet** is generated by `sync-brd.py` directly from `06_INFORMATION_ARCHITECTURE/notifications.md` — this mode contributes by adding new NOTIF-NNN entries to that file when error or behavior flows require new messages

Run `python design/scripts/sync-brd.py` after updating story-map.md AC bullets to regenerate the BRD User Stories sheet (with [BR-NN] inline-expansion and [NOTIF-NNN] preserved as references).
